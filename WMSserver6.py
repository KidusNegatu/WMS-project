...
class WMSDataProcessing():
    def __init__(self, port, baud, latitude, longitude):
        ...
        self.currentUrl = (
            ...
            f"?key=YOUR_WEATHER_API_KEY&q={latitude},{longitude}"
        )
        ...
        self.user = Client()
        self.user.set_endpoint("https://nyc.cloud.appwrite.io/v1")
        self.user.set_key("YOUR_APPWRITE_API_KEY")
        self.user.set_project("42914291")
    ...
    def serialCom(self):
        try:
            with serial.Serial(self.port, self.baud, timeout=1) as com:
                ...
                # Give the device time to initialize after opening serial
                time.sleep(2)

                buffer = ""

                while True:
                    chunk = com.read(com.in_waiting or 1).decode(
                        "utf-8",
                        errors="ignore"
                    )
                    ...
                    if chunk:
                        ...
                        # Remove anything before the JSON object
                        if start > 0:
                            buffer = buffer[start:]
                        ...
                        # Remove the JSON we just processed
                        buffer = buffer[end + 1:]
    ...
    def weatherAPIData(self):
        ...
        try:
            ...
            print(
                "SOMETHING WENT WRONG WHILE READING WEATHER API DATA!!",
                err
            )
            self.failed = True
    ...
    def saveDataToServer(self):
        ...
        try:
            ...
            print(
                "SOMETHING WENT WRONG WHILE UPLOADING DATA TO APPWRITE!!",
                err
            )
            self.failed = True
